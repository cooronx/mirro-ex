...
const TRADING_EMPTY_USERNAME_CODE: i32 = 2107;
const TRADING_EMPTY_PASSWORD_CODE: i32 = 2108;
...
const TRADING_INVALID_CREDENTIALS_CODE: i32 = 2407;
...
pub fn router(trading_store: Arc<TradingStore>, replay_manager: Arc<ReplayManager>) -> Router {
    Router::with_path("trading")
        ...
        .push(Router::with_path("login").post(LoginHandler {
            trading_store: trading_store.clone(),
        }))
...
impl Handler for LoginHandler {
    async fn handle(
        ...
        let Some(request) = parse_json_body::<LoginRequest>(
            req,
            res,
            TRADING_INVALID_LOGIN_REQUEST_CODE,
        ...
        let trading_store = self.trading_store.clone();
        match task::spawn_blocking(move || trading_store.login(request)).await {
            Ok(Ok(account)) => res.render(Json(ApiResponse::success(account))),
            Ok(Err(err)) => render_trading_error(res, err),
            Err(err) => {
                render_api_error(
                    res,
                    TRADING_INTERNAL_TASK_CODE,
                    format!("login task join failed: {err}"),
                );
            }
        }
    }
...
fn render_trading_error(res: &mut Response, err: TradingStoreError) {
    let code = match err {
        TradingStoreError::InvalidUserId => TRADING_INVALID_USER_ID_CODE,
        TradingStoreError::EmptyUsername => TRADING_EMPTY_USERNAME_CODE,
        TradingStoreError::EmptyPassword => TRADING_EMPTY_PASSWORD_CODE,
        ...
        TradingStoreError::InvalidCredentials => TRADING_INVALID_CREDENTIALS_CODE,
        ...
        | TradingStoreError::QueryAccountByUsername { .. }
